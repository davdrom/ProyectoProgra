...
for temporada in temporada_options[0:]:
    ...
    temporada.click()
    time.sleep(2)  # Agregamos un tiempo de espera para asegurarnos de que la página se cargue completamente

    # Buscamos en la primera tabla
    ...
    # El html venia dividido en 2 tablas
    # Buscamos en la segunda tabla
    tabla_datos = soup.find("table", attrs={"class": "Table Table--align-right"})
    filas_datos = tabla_datos.find_all("tr")

    # Recorremos las filas de la tabla de segunda tabla y extraemos la información
...
navegador.quit()  # Cerrar el navegador
